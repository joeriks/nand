...
export function Workbench({ opened, user, dark, onTheme, onWorkspace, onLogout, onHome, desktop = false, onReconnect, localFolder, onLocalFolder, initialFile }: {
  ...
  }, [account, scope, store, local, setNotice, setError]);
  ...
  }, [active, local, files, lockedKey, store, runSync, setCompare, setNotice, setError]);
  useEffect(() => {
  ...
  const stats = useMemo(() => image ? { words: 0, chars: 0 } : ({ words: draft?.text.trim().split(/\s+/).filter(Boolean).length || 0, chars: draft?.text.length || 0 }), [draft?.text, image]);
  ...
  return <div className="workbench" inert={choosingFolder}>
    ...
    <main className="workspace-main">
      <header className="workspace-toolbar"><div className="breadcrumbs"><button className="icon-button mobile-only" onClick={() => setSidebar(true)} aria-label="Öppna navigation"><Menu size={20} /></button><FileText size={16} /><span>{draft?.path || "Din arbetsyta"}</span></div><div className="toolbar-actions">{desktop && updateState.phase === "available" && <button className="text-button" onClick={() => setUpdatesOpen(true)}>Ny version</button>}{!image && <div className="view-switch" aria-label="Visningsläge">{(plainText ? [["edit", "Text"]] as const : csv ? [["edit", "Tabell"], ["preview", "CSV-text"]] as const : [["edit", "Skriv"], ["split", "Delad vy"], ["preview", "Läs"]] as const).map(([value, label]) => <button key={value} className={(plainText || (csv && mode === "split") ? "edit" : mode) === value ? "selected" : ""} aria-pressed={(plainText || (csv && mode === "split") ? "edit" : mode) === value} onClick={() => setMode(value)}>{label}</button>)}</div>}<ActionMenu>{desktop && <button onClick={() => void newWindow()}><Layers3 size={16} />Nytt fönster</button>}
        ...
        <button onClick={() => void exportDraft()} disabled={!draft}><Download size={16} />{csv ? "Exportera CSV" : image ? "Exportera bild" : plainText ? "Exportera TXT" : "Exportera Markdown"}</button>
      ...
      <div className="editor-and-info">{draft && image && <Suspense fallback={<p className="editor-loading">Öppnar bilden…</p>}><ImageEditor key={draft.key} value={draft.text} fileName={draft.path.split("/").pop() || "bild.png"} onExport={async dataUrl => { try { const { invoke } = await import("@tauri-apps/api/core"); return await invoke<boolean>("export_image", { name: draft.path.split("/").pop(), dataUrl }); } catch (cause) { setError(typeof cause === "string" ? cause : "Bilden kunde inte exporteras."); return false; } }} onChange={text => store.update(draft.key, current => ({ ...current, text, updatedAt: Date.now() }))} readOnly={locked || !!draft.conflict} /></Suspense>}<div className={`document-body mode-${plainText || image ? "edit" : mode} ${image ? "image-hidden" : ""}`}>
      ...
      </div>{details && <aside className="info-panel"><div className="info-heading"><Info size={16} /><h2>Om anteckningen</h2><button className="icon-button" onClick={() => setDetails(false)} aria-label="Stäng information"><X size={16} /></button></div><dl><dt>Format</dt><dd>{image ? "Bild" : csv ? "CSV" : plainText ? "TXT" : "Markdown"}{image ? "" : " · UTF-8"}</dd><dt>Sökväg</dt><dd>{draft?.path || "—"}</dd><dt>Lagring</dt><dd>{local ? (desktop ? files?.state.directory || localFolder?.directory || "Dokument/nand" : "Den här webbläsaren") : `${wiki ? "GitHub Wiki" : "Repositoryfiler"} · ${opened.workspace.repository.fullName}`}</dd>{!local && <><dt>Gren</dt><dd>{opened.workspace.branch}</dd></>}<dt>Ord</dt><dd>{stats.words}</dd></dl><p className="hint">Lokala utkast är inte en permanent säkerhetskopia. Webbläsardata kan rensas.</p><p className="hint">Länkar, bakåtlänkar och egenskapsvyer kommer i nästa etapp.</p></aside>}</div>
      <footer className="statusbar"><span className={`save-state ${localStatus === "failed" || draft?.conflict ? "warning" : ""}`} role="status">{!online ? <WifiOff size={14} /> : busy || localStatus === "writing" ? <LoaderCircle size={14} className="spin" /> : <Check size={14} />}{draft ? status : "Redo för din nästa tanke"}</span><span className="status-stats">{!local && <span><GitBranch size={13} />{opened.workspace.branch}</span>}{!image && <><span>{stats.words} ord</span><span>{stats.chars} tecken</span></>}<span>{image ? "Bild" : csv ? "CSV" : plainText ? "TXT" : "Markdown"}</span></span></footer>
    </main>
