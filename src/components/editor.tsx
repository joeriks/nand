...
export default function Editor({ value, onChange, onSave, readOnly, dark, format = "markdown" }: {
  value: string; onChange: (value: string) => void; onSave: () => void; readOnly: boolean; dark: boolean; format?: "markdown" | "text";
}) {
  ...
  const extensions = useMemo(() => [...(format === "markdown" ? [markdown()] : []), EditorView.lineWrapping,
    ...
    EditorView.contentAttributes.of({ "aria-label": format === "markdown" ? "Anteckningens innehåll" : "CSV-filens innehåll", spellcheck: format === "markdown" ? "true" : "false" }),
